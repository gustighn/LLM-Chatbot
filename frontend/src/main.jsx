...
function App() {
  ...
  const findMatch = async () => {
    ...
    if (id) {
      setMatchId(id);
      const qs = await backend.getQuestions(id);
      setQuestions(qs);
    } else {
  ...
  // Submit answers
  const submit = async () => {
    await backend.submitAnswer(matchId, answers);
    const res = await backend.getResult(matchId);
    setResult(res);
    // Update profile
    const prof = await backend.getProfile();
    setProfile(prof);
  ...
  // Render logic (form login, soal, hasil, dsb)
  // ...

  return (
    <div>
      {/* Render login, profile, matchmaking, questions, answers, result */}
    </div>
  );
}
